...
#[cfg(target_os="windows")]
#[tauri::command] async fn set_windows_password(state: State<'_, Arc<AppState>>,password:String)->Result<(),String>{use wristkey_core::vault::KeyProtector;use wristkey_platform_win::WindowsKeyProtector;let devices=state.session.list_paired_devices().await.map_err(|e|e.to_string())?;let device=devices.first().ok_or("No paired device found. Pair a watch first.")?;let encrypted=WindowsKeyProtector.protect(password.as_bytes());state.session.set_device_password(device.id,encrypted).await.map_err(|e|e.to_string())}
