...
use wristkey_daemon::{Daemon, ConnectionManager, wait_for_response};
...
#[tauri::command] async fn set_windows_password(state: State<'_, Arc<AppState>>,password:String)->Result<(),String>{
    ...
    if password.is_empty() {
        return Err("Windows password cannot be empty.".into());
    }
    WindowsSecurity::validate_current_user_password(&password)?;
    let devices=state.session.list_paired_devices().await.map_err(|e|e.to_string())?;
    let device=devices.first().ok_or("No paired device found. Pair a watch first.")?;
    let encrypted=WindowsKeyProtector.protect(password.as_bytes());
...
#[cfg(target_os="windows")]
#[tauri::command] async fn clear_windows_password(state: State<'_, Arc<AppState>>)->Result<(),String>{let devices=state.session.list_paired_devices().await.map_err(|e|e.to_string())?;let device=devices.first().ok_or("No paired device found. Pair a watch first.")?;state.session.clear_device_password(device.id).await.map_err(|e|e.to_string())}
#[cfg(not(target_os="windows"))]
...
#[tauri::command] async fn register_credential_provider(app: tauri::AppHandle)->Result<(),String>{
    ...
    if let Some(resolved)=WindowsSecurity::resolve_credential_provider_dll(){candidates.push(resolved);}
...
#[derive(serde::Serialize, Clone)] struct TrainingStatusDto { state: String, countdown: Option<i32>, samples: Option<i32>, zone_rssi: Option<i32>, rssi: Option<i32> }
...
#[tauri::command] async fn subscribe_watch_training(state: State<'_, Arc<AppState>>)->Result<(),String>{let devices=state.session.list_paired_devices().await.map_err(|e|e.to_string())?;let device=devices.first().ok_or("No paired device")?;let service_uuid=uuid::Uuid::parse_str(SERVICE_UUID).map_err(|e|e.to_string())?;let training_uuid=uuid::Uuid::parse_str(TRAINING_CONTROL_CHAR).map_err(|e|e.to_string())?;let info=PeripheralInfo{id:device.address.clone(),name:Some(device.name.clone()),pin:None,device_id:device.device_id.as_ref().and_then(|v|String::from_utf8(v.clone()).ok()),rssi:None,service_uuids:vec![service_uuid],raw_manufacturer_data:None};let conn=state.conn_mgr.get_or_connect(&state.ble,&info).await.map_err(|e|format!("BLE connect failed: {}",e))?;let mut rx=state.ble.notify(&conn,training_uuid).await.map_err(|e|format!("BLE subscribe failed: {}",e))?;let state_store=TRAINING_STATE.get_or_init(||std::sync::Mutex::new(TrainingStatusDto{state:"idle".into(),countdown:None,samples:None,zone_rssi:None,rssi:None}));tokio::spawn(async move{while let Ok(data)=rx.recv().await{if let Ok(json)=serde_json::from_slice::<serde_json::Value>(&data){let mut s=state_store.lock().unwrap();s.state=json.get("state").and_then(|v|v.as_str()).unwrap_or("unknown").to_string();s.countdown=json.get("countdown").and_then(|v|v.as_i64()).map(|v|v as i32);s.samples=json.get("samples").and_then(|v|v.as_i64()).map(|v|v as i32);s.zone_rssi=json.get("zone_rssi").and_then(|v|v.as_i64()).map(|v|v as i32);s.rssi=json.get("rssi").and_then(|v|v.as_i64()).map(|v|v as i32);info!("Watch training state: {} countdown={:?} samples={:?} zone_rssi={:?} rssi={:?}",s.state,s.countdown,s.samples,s.zone_rssi,s.rssi)}}});Ok(())}

#[tauri::command] async fn get_watch_training_status()->Result<TrainingStatusDto,String>{let store=TRAINING_STATE.get_or_init(||std::sync::Mutex::new(TrainingStatusDto{state:"idle".into(),countdown:None,samples:None,zone_rssi:None,rssi:None}));Ok(store.lock().unwrap().clone())} 
mod log_rolling;
...
async fn main() {
    #[cfg(windows)]
    {
        ...
        if args.iter().any(|a| a == "--register-cp") {
            match WindowsSecurity::resolve_credential_provider_dll() {
                Some(dll) => match WindowsSecurity::register_credential_provider(dll.to_string_lossy().as_ref()) {
                    Ok(()) => { println!("WristKey Credential Provider registered."); std::process::exit(0); }
                    Err(e) => { println!("WristKey Credential Provider registration failed: {}", e); std::process::exit(1); }
                },
                None => { println!("WristKey Credential Provider DLL not found."); std::process::exit(2); }
            }
        }
        if args.iter().any(|a| a == "--service") {
            if let Some(i) = args.iter().position(|a| a == "--data-dir") {
                if let Some(dir) = args.get(i + 1) {
                    std::env::set_var("WRISTKEY_DATA_DIR", dir);
                }
            }
