...
use wristkey_core::{Config, SessionManager, EcdsaP256Crypto, SledStorage, PlatformSecurity, Response};
...
#[tauri::command]
async fn calibrate_device(app: tauri::AppHandle, state: State<'_, Arc<AppState>>, id: String) -> Result<CalibrationResultDto, String> {
    let device = state.session.list_paired_devices().await.map_err(|e| e.to_string())?.into_iter().find(|d| d.id.to_string() == id).ok_or_else(|| "Device not found".to_string())?;
    let result = calibration::run(&app, state.session.clone(), state.ble.clone(), id, device.address, 60, 15).await?;
    Ok(CalibrationResultDto { avg: result.median, threshold: result.threshold, samples: result.samples })
}
...
async fn main() {
    ...
    let storage: Arc<dyn wristkey_core::Storage> = match SledStorage::open_default() {
        Ok(storage) => { info!("Persistent storage opened; paired watches will survive restarts"); Arc::new(storage) }
        Err(e) => { error!("Failed to open persistent storage: {}", e); return; }
    };
    ...
    tauri::Builder::default()
        ...
        .expect("error while running WristKey");
}