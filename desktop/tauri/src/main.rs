...
async fn main() {
    ...
    // Persist paired watches in the user's WristKey data directory. Sled survives
    // application restarts, unlike the old in-memory storage.
    let storage: Arc<dyn wristkey_core::Storage> = match SledStorage::open_default() {
        Ok(storage) => {
            info!("Persistent storage opened; paired watches will survive restarts");
            Arc::new(storage)
        }
        Err(e) => {
            error!("Failed to open persistent storage: {}", e);
            return;
        }
    };
