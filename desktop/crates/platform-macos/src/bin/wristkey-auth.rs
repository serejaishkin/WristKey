...
// This binary uses macOS/Unix-only IPC primitives. Keep the real implementation
// behind a target cfg so the workspace can still be checked from Windows/Linux.
#[cfg(target_os = "macos")]
mod macos_auth {
...
fn main() {
    eprintln!("wristkey-auth is only available on macOS");
}
