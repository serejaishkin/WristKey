...
public:
    ...
    ULONG STDMETHODCALLTYPE Release() override { ULONG count = InterlockedDecrement(&ref_); if (!count) delete this; return count; }
    ...
    HRESULT STDMETHODCALLTYPE GetProviderID(GUID* providerID) override {
        if (!providerID) return E_POINTER; *providerID = Identity_LocalUserProvider; return S_OK;
    }
...
public:
    explicit FakeUserArray(ICredentialProviderUser* user) : user_(user) { if (user_) user_->AddRef(); }
    ~FakeUserArray() { if (user_) user_->Release(); }
    HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, void** ppv) override {
        if (!ppv) return E_POINTER;
        *ppv = nullptr;
    ...
    HRESULT STDMETHODCALLTYPE GetAt(DWORD index, ICredentialProviderUser** user) override {
        ...
        *user = nullptr;
        ...
        user_->AddRef(); *user = user_; return S_OK;
    }
private:
    LONG ref_ = 1;
...
int wmain(int argc, wchar_t** argv) {
    ...
    auto cleanup = [&]() { FreeLibrary(module); CoUninitialize(); };

    auto getClassObject = reinterpret_cast<DllGetClassObjectFn>(GetProcAddress(module, "DllGetClassObject"));
    if (!getClassObject) { std::wcerr << L"DllGetClassObject export not found.\n"; cleanup(); return 5; }

    IClassFactory* factory = nullptr;
    hr = getClassObject(CLSID_WristKeyCredentialProvider, IID_IClassFactory, reinterpret_cast<void**>(&factory));
    if (FAILED(hr) || !factory) { int code = Fail(L"DllGetClassObject failed:", hr, 6); cleanup(); return code; }

    ICredentialProvider* provider = nullptr;
    hr = factory->CreateInstance(nullptr, IID_ICredentialProvider, reinterpret_cast<void**>(&provider));
    factory->Release();
    if (FAILED(hr) || !provider) { int code = Fail(L"CreateInstance failed:", hr, 7); cleanup(); return code; }

    hr = provider->SetUsageScenario(CPUS_UNLOCK_WORKSTATION, 0);
    if (FAILED(hr)) { int code = Fail(L"SetUsageScenario failed:", hr, 8); provider->Release(); cleanup(); return code; }

    auto* fakeUser = new (std::nothrow) FakeUser(L"WristKeyTestUser", L"WristKeyTestUser", L"S-1-5-21-111111111-222222222-333333333-1001");
    if (!fakeUser) { provider->Release(); cleanup(); return 9; }
    auto* fakeArray = new (std::nothrow) FakeUserArray(fakeUser);
    if (!fakeArray) { fakeUser->Release(); provider->Release(); cleanup(); return 10; }

    ICredentialProviderSetUserArray* setUsers = nullptr;
    hr = provider->QueryInterface(IID_ICredentialProviderSetUserArray, reinterpret_cast<void**>(&setUsers));
    if (FAILED(hr) || !setUsers) { int code = Fail(L"QI SetUserArray failed:", hr, 11); fakeArray->Release(); fakeUser->Release(); provider->Release(); cleanup(); return code; }

    hr = setUsers->SetUserArray(fakeArray);
    setUsers->Release(); fakeArray->Release(); fakeUser->Release();
    if (FAILED(hr)) { int code = Fail(L"SetUserArray failed:", hr, 12); provider->Release(); cleanup(); return code; }

    DWORD count = 0, defaultIndex = CREDENTIAL_PROVIDER_NO_DEFAULT;
    BOOL autoLogon = FALSE;
    hr = provider->GetCredentialCount(&count, &defaultIndex, &autoLogon);
    if (FAILED(hr)) { int code = Fail(L"GetCredentialCount failed:", hr, 13); provider->Release(); cleanup(); return code; }
    if (count != 1) { std::wcerr << L"Unexpected credential count: " << count << L" (expected 1)\n"; provider->Release(); cleanup(); return 14; }

    ICredentialProviderCredential* credential = nullptr;
    hr = provider->GetCredentialAt(0, &credential);
    if (FAILED(hr) || !credential) { int code = Fail(L"GetCredentialAt failed:", hr, 15); provider->Release(); cleanup(); return code; }

    ICredentialProviderCredential2* credential2 = nullptr;
    hr = credential->QueryInterface(IID_ICredentialProviderCredential2, reinterpret_cast<void**>(&credential2));
    if (FAILED(hr) || !credential2) { int code = Fail(L"QI ICredentialProviderCredential2 failed:", hr, 16); credential->Release(); provider->Release(); cleanup(); return code; }

    PWSTR sid = nullptr;
    hr = credential2->GetUserSid(&sid);
    if (FAILED(hr) || !sid) { int code = Fail(L"GetUserSid failed:", hr, 17); credential2->Release(); credential->Release(); provider->Release(); cleanup(); return code; }

    const std::wstring expectedSid = L"S-1-5-21-111111111-222222222-333333333-1001";
    if (expectedSid != sid) { std::wcerr << L"Unexpected SID: " << sid << L"\n"; CoTaskMemFree(sid); credential2->Release(); credential->Release(); provider->Release(); cleanup(); return 18; }

    PWSTR name = nullptr;
    hr = credential->GetStringValue(1, &name);
    if (FAILED(hr) || !name) { int code = Fail(L"GetStringValue(Tile) failed:", hr, 19); CoTaskMemFree(sid); credential2->Release(); credential->Release(); provider->Release(); cleanup(); return code; }
    ...
    CoTaskMemFree(name); CoTaskMemFree(sid);
    credential2->Release(); credential->Release(); provider->Release(); cleanup();
    return 0;
}
