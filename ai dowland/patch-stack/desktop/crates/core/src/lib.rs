...
impl SessionManager {
    ...
    /// Persists a new baseline_rssi for a paired device — used by calibration.
    /// Without this, calibrating only updated the watch's own local copy
    /// (WristKeySettings on the Kotlin side); the desktop's own proximity
    /// threshold check (see Daemon::check_proximity) never changed.
