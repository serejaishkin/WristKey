...
impl Daemon {
    ...
    pub async fn calibrate_proximity(&self, device_id: Uuid) -> Result<i16> {
        ...
        // Must match CONFIG_CHAR_UUID in WristKeyBleService.kt exactly — this is a
        // separate characteristic from CHALLENGE/RESPONSE/STATUS, specifically for
        // calibration commands (CMD_START_CALIBRATION / CMD_CALIBRATION_RESULT /
        // CMD_CANCEL_CALIBRATION). Using the STATUS characteristic here (as an
        // earlier draft of this function did) would fail on real hardware — STATUS
        // is READ|NOTIFY|INDICATE only, not writable.
