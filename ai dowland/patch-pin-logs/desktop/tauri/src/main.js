...
// Diagnostics: show where logs actually are, since the file name isn't
// what you'd guess (wristkey.YYYY-MM-DD, not wristkey.log).
(async () => {
  try {
    const dir = await invoke('get_log_dir');
    const el = document.getElementById('logDirDisplay');
    if (el) el.value = dir;
  } catch (err) {
    console.error('get_log_dir failed:', err);
    const el = document.getElementById('logDirDisplay');
    if (el) el.value = '(unavailable)';
  }
})();

document.getElementById('copyLogDirBtn')?.addEventListener('click', async () => {
  const el = document.getElementById('logDirDisplay');
