...
/// Set once in main() right after the log directory is resolved, so the
/// get_log_dir command below can expose it to the UI without threading it
/// through Tauri's managed state everywhere.
...
fn main() {
    ...
    // NOT "wristkey.log". The message below used to point at a filename that
    // never actually gets created, which is very likely why logs seemed to
    // "not write" — they were writing, just under a different name than
    ...
    println!("[WristKey] Today's log file: {:?}", actual_log_path);

    // Bootstrap line written directly (not through tracing) so that even a
    // crash before the subscriber below is initialized — e.g. SQLite failing
    // to open, a permissions error — leaves *something* on disk to look at,
    // instead of only ever reaching an eprintln! that's invisible when the
    // app is launched without an attached console (the normal case for a
    // double-clicked .exe).
    {
        use std::io::Write;
    ...
    let (file_layer, _file_guard) = if config.log_to_file {
        ...
        println!("[WristKey] File logging enabled: {:?}", actual_log_path);
