...
namespace WristKeyCredentialProvider
{
    ...
    public enum CREDENTIAL_PROVIDER_USAGE_SCENARIO : uint
    {
        CPUS_INVALID = 0,
        CPUS_LOGON = 1,
    ...
    public enum CREDENTIAL_PROVIDER_FIELD_TYPE : uint
    {
        ...
        CPFT_VALIDATION_SMALL_TEXT = 10,
        CPFT_VISUAL_CUE = 11,
        CPFT_LABEL = 12,
        CPFT_BITMAP = 13,
        CPFT_PASSWORD_TEXT_WITH_BUTTON = 14,
        CPFT_CREDENTIAL_PROVIDER_LOGO = 15,
        CPFT_CREDENTIAL_PROVIDER_LABEL = 16
    }

    public enum CREDENTIAL_PROVIDER_FIELD_STATE : uint
    {
        CPFS_HIDDEN = 0,
    ...
    public enum CREDENTIAL_PROVIDER_FIELD_INTERACTIVE_STATE : uint
    {
        CPFIS_NONE = 0,
        CPFIS_READONLY = 1,
        CPFIS_DISABLED = 2,
    ...
    public enum CREDENTIAL_PROVIDER_GET_SERIALIZATION_RESPONSE : uint
    {
    ...
    public enum CREDENTIAL_PROVIDER_STATUS_ICON : uint
    {
        CPSI_NONE = 0,
        CPSI_ERROR = 1,
        CPSI_WARNING = 2,
    ...
    public interface ICredentialProviderCredential
    {
        ...
        int SetStringValue(uint dwFieldID, string psz);
        int SetCheckboxValue(uint dwFieldID, int bChecked);
        int SetComboBoxSelectedValue(uint dwFieldID, uint dwSelectedItem);
        int DeleteComboBoxValue(uint dwFieldID, uint dwItem);
        int SetSubmitButtonValue(uint dwFieldID, uint dwAdjacentTo);
        int GetSerialization(out CREDENTIAL_PROVIDER_GET_SERIALIZATION_RESPONSE pcpgsr,
            out CREDENTIAL_PROVIDER_CREDENTIAL_SERIALIZATION pcpcs,
            out IntPtr ppszOptionalStatusText,
    ...
    {
        int CredentialsChanged(UIntPtr upAdviseContext);
    }
    ...
    {
        // This CLSID is deliberately stable: the installer registers it under
        // Authentication\Credential Providers so LogonUI can enumerate the tile.
        ...
        public int GetFieldDescriptorAt(uint dwIndex, out IntPtr ppcpfd)
        {
            ...
            var fd = new CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR
            {
                ...
                pszLabel = Marshal.StringToCoTaskMemUni(dwIndex == 0 ? "WristKey" : "Status"),
    ...
    public sealed class WristKeyCredential : ICredentialProviderCredential
    {
        ...
        public int GetFieldState(uint dwFieldID, out CREDENTIAL_PROVIDER_FIELD_STATE pcpfs, out CREDENTIAL_PROVIDER_FIELD_INTERACTIVE_STATE pcpfis)
        {
            if (dwFieldID > 1)
            {
                pcpfs = CREDENTIAL_PROVIDER_FIELD_STATE.CPFS_HIDDEN;
                pcpfis = CREDENTIAL_PROVIDER_FIELD_INTERACTIVE_STATE.CPFIS_NONE;
                return HResult.E_INVALIDARG;
            }
            pcpfs = CREDENTIAL_PROVIDER_FIELD_STATE.CPFS_DISPLAY_IN_BOTH;
            pcpfis = CREDENTIAL_PROVIDER_FIELD_INTERACTIVE_STATE.CPFIS_NONE;
            return HResult.S_OK;
        }
        ...
        {
            ppsz = Marshal.StringToCoTaskMemUni(dwFieldID == 0 ? "WristKey" : "Bring your watch close");
            return dwFieldID <= 1 ? HResult.S_OK : HResult.E_INVALIDARG;
        }
        ...
        public int SetStringValue(uint dwFieldID, string psz) => HResult.E_NOTIMPL;
        public int SetCheckboxValue(uint dwFieldID, int bChecked) => HResult.E_NOTIMPL;
        public int SetComboBoxSelectedValue(uint dwFieldID, uint dwSelectedItem) => HResult.E_NOTIMPL;
        public int DeleteComboBoxValue(uint dwFieldID, uint dwItem) => HResult.E_NOTIMPL;
        public int SetSubmitButtonValue(uint dwFieldID, uint dwAdjacentTo) => HResult.E_NOTIMPL;

        // Phase A: deliberately do not authenticate yet. The tile must first be
        // proven to load safely in LogonUI. Phase B will replace this with the
        // real WristKey challenge -> Kerberos/Winlogon serialization.
